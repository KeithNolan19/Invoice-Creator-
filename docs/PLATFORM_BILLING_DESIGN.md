...
| D6 | **Plans** | ✅ Three tiers, priced by user count. Yearly = 12 months − 5%. |
| D9 | **Tier changes** | ✅ **Admin-controlled** — you assign the plan; the system *notifies* you when a tenant outgrows it, never changes the charge automatically. |
| D10 | **Over 15 users** | ✅ Not modelled yet — the three tiers only. |

### Subscription plans (seeded in migration 011)

| code | name | max users | € / month | € / year (−5%) |
| --- | --- | --- | --- | --- |
| `starter` | Starter | 5 | 10.00 | 114.00 |
| `team` | Team | 10 | 15.00 | 171.00 |
| `business` | Business | 15 | 20.00 | 228.00 |

`tenant_subscriptions.billing_interval` (`month` | `year`) picks the cadence;
the yearly amount is `monthly_cents × 12 × (1 − yearly_discount_pct/100)` with
`yearly_discount_pct` (default 5) on `platform_billing_config`. The resolved
`amount_cents` is snapshotted onto the subscription and each invoice.

**Outgrown-plan handling:** the billing tick raises an `admin_notification`
(`type = 'plan_outgrown'`, deduped per tenant+plan) when a tenant's active user
count exceeds `plan.max_users`. It never changes the plan or the invoice — you
bump them from the admin billing screen.
